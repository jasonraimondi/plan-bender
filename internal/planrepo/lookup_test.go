...
func TestFindIssueProject_SkipsHiddenAndArchive(t *testing.T) {
	...
	require.NoError(t, os.WriteFile(filepath.Join(archivedIssuesDir, "7-x.json"), []byte(issueJSON(7, "x")), 0o644))

	writePlan(t, plansDir, "live", validPrd, map[string]string{
		"7-y.json": issueJSON(7, "y"),
	})
